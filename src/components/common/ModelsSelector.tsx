...
import {Brain, Search, Loader2, HelpCircle, Sparkles, AlertCircle} from 'lucide-react';
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-64 pl-9 pr-4 py-1.5 bg-white/10 text-white placeholder-primary-300 rounded-lg border border-primary-400/30 focus:outline-none focus:ring-2 focus:ring-primary-400 focus:border-transparent"
            />
          </div>
        </div>
              ...
              <AnimatePresence>
                {showTooltip === type.id && (
                  <motion.div
                    ...
                    exit={{opacity: 0, y: 5}}
                    className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2 bg-gray-900 text-white text-xs rounded-lg shadow-xl"
                  >
                    {type.description}
                    <div
                      className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 rotate-45 w-2 h-2 bg-gray-900"/>
                  </motion.div>
                )}
              </AnimatePresence>
            </button>
            ...
            {loadingStates[activeTab] ? (
              <div className="flex items-center justify-center py-12">
                <div className="text-center space-y-3">
                  ...
                  <p
                    className="text-primary-900">Loading {modelTypes.find(t => t.id === activeTab)?.label} models...</p>
                </div>
              </div>
            ) : errors[activeTab] ? (
